...
export default class PikkuToneAPI {
  ...
  // initializeScriptProcessor メソッド
  initializeScriptProcessor() {
      try {
          const bufferSize = 2048; // バッファサイズを調整可能に
  ...
  // loadAudioBuffers メソッド
  async loadAudioBuffers(audioData) {
      try {
          const buffers = new Map();
          for (const [phoneme, bufferData] of Object.entries(audioData)) {
              const audioBuffer = await this.audioContext.decodeAudioData(bufferData);
              buffers.set(phoneme, audioBuffer);
          }
          return buffers;
      } catch (error) {
          throw new PikkuToneError(401, "AudioBufferのロードに失敗しました", {
              originalError: error,
          });
      }
  }
  ...
  async saveToAbuf(buffers) {
      try {
          ...
          for (const [phoneme, buffer] of buffers) {
              const channelData = [];
              for (let channel = 0; channel < buffer.numberOfChannels; channel++) {
                  channelData.push(buffer.getChannelData(channel));
              }
              abufData.buffers[phoneme] = channelData;
          }

          const blob = new Blob([JSON.stringify(abufData)], {
              type: "application/abuf",
          });
          return blob;
      } catch (error) {
          throw new PikkuToneError(401, "abufファイルの保存に失敗しました", {
              originalError: error,
          });
      }
  }
  ...
  // loadFromAbuf メソッド
  async loadFromAbuf(abufBlob) {
      try {
          const abufData = JSON.parse(await abufBlob.text());
          const buffers = new Map();

          for (const [phoneme, channelData] of Object.entries(abufData.buffers)) {
          ...
          throw new PikkuToneError(401, "abufファイルのロードに失敗しました", {
              originalError: error,
          });
      }
  }

  // startProcessing メソッド
 startProcessing(audioBufferToPlay) {
      if (!audioBufferToPlay) {
          console.warn("No audio buffer provided to startProcessing.");
          return;
      }
      if (this.isPlaying) {
      ...
      this.sourceNode.buffer = audioBufferToPlay;
  ...
  // stopProcessing メソッド (役割変更: 再生停止)
  stopProcessing() {
      if (this.sourceNode) {
          this.sourceNode.stop(); // stop() は非同期で再生終了を待つ
          // stop 後に disconnect する
          this.sourceNode.onended = null; // イベントリスナーをクリア
